...
def calculate_robustness(G):
    return (1 - calculate_vulnerability_function(G)) * 2 * calculate_efficiency(G) * calculate_critical_threshold(G)
...
df = df.sort_values(by='Robustness', ascending=False)  # Sort the DataFrame by 'Robustness' column in descending order
